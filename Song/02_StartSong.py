...
instr08 = SimpleUDPClient(ip, recPort+8)
instr09 = SimpleUDPClient(ip, recPort+9)
instr10 = SimpleUDPClient(ip, recPort+10)
...
#dispatcher.set_default_handler(set_filter)
# Setup the server
...
while ((time.time() - start_time) < 25.0):
    ...
    server.handle_timeout(0.1)
    print(f"While {start_time}")
